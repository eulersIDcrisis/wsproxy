...
Test cases for the basic proxy routes.
"""
import unittest
import uuid
import asyncio
...
from wsproxy.routes import proxy as proxy_routes
...
class WebsocketServerTest(testing.AsyncHTTPTestCase):
    ...
    def get_app(self):
        routes = proxy_routes.get_routes()
    ...
    async def test_proxy_route(self):
        with contextlib.ExitStack() as exit_stack:
            ...
            # async with proxy_request.RemoteSocket(
            #     state, 'localhost', remote_port
            # ) as remote_socket:
            #     pass

            socket_id = uuid.uuid1()
            handler = MockHandler()
            state.add_proxy_socket(socket_id, handler)
            exit_stack.callback(state.remove_proxy_socket, socket_id)
            async with json_request.setup_subscription(
                ...
                # Now, setup a separate subscription to the monitor proxy.
                monitor_sub = json_request.setup_subscription(
                    state, 'proxy_socket_monitor', dict(socket_id=socket_id.hex))
                msg = await monitor_sub.next()
                print("MSG: ", msg)
